...
public class GolfStateComparator extends StateComparator<GolfState> {
	...
	public boolean isStateExplored(GolfState searchState) {
		...
		int xStatePos = (int) (exploredStatesData[0].length / 2 - (searchState.getBall().getComponent(Position.class).x / unitSize));
		int yStatePos = (int) ((exploredStatesData.length / 2 - searchState.getBall().getComponent(Position.class).y
				/ unitSize));
	...
	public void markExplored(GolfState searchState) {
		...
		if(xStatePos < 0) {
			...
			Position position = searchState.getBall().getComponent(Position.class);
